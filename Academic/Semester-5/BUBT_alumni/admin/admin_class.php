...
Class Action {
	...
	function update_donation(){
		extract($_POST);
		$update = $this->db->query("UPDATE donations set status = $status where id = $id");
		if($update)
			return 1;
	}
	...
	function save_forum(){
		...
		if(empty($id)){
			// echo "INSERT INTO careers set ".$data;
		$data .= ", user_id = '{$_SESSION['login_id']}' ";
			$save = $this->db->query("INSERT INTO donations set ".$data);
		}else{
			$save = $this->db->query("UPDATE donations set ".$data." where id=".$id);
		}
		if($save)
			return 1;
	}
	...
	function delete_donation(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM donations where id = ".$id);
		if($delete){
			return 1;
		}
	}
